...
import { Check, ChevronsUpDown, Pencil, icons } from "lucide-react";
...
export function UpdateSubCategory({ subcategoryId }: UpdateSubCategoryProps) {
  ...
  const { data: categories } = trpc.getServicenowCategories.useQuery();

  const { data: subcategory } = trpc.getServicenowSubCategory.useQuery({
    id: subcategoryId,
  });

  const { mutate: updateServicenowSubCategory, isLoading } =
    trpc.updateServicenowSubCategory.useMutation({
      onSuccess: () => {
  ...
  function onSubmit(data: ServicenowSubCategoryValues) {
    updateServicenowSubCategory({ id: subcategoryId, ...data });
  }
      ...
      <DialogContent className="sm:max-w-md">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
            <DialogHeader>
              <DialogTitle>{`Modification d'une sous-catégorie`}</DialogTitle>
            </DialogHeader>
